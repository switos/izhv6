        ...
        { // Main Window Area
            ...
            GUILayout.BeginVertical();
            {
                ...
                GUILayout.BeginHorizontal();
                {
                    ...
                    // Check if the GUI changed (slider moved)
                    if (GUI.changed)
                    {
                        // Update the available currency using the modified value
                        InventoryManager.Instance.availableCurrency = currency;
                    }
                }
                // End the horizontal layout block
                GUILayout.EndHorizontal();
                ...
                // Handle for mouse interaction
                GameManager.Instance.interactiveMode = GUILayout.Toggle(
                    GameManager.Instance.interactiveMode, "Mouse Interaction"
                );

                // Handle for master volume control
                ...
                GUILayout.Label($"Master Volume: {SoundManager.Instance.masterVolume:F2} dB");

                // Handle for muting sound
                bool soundMuted = SoundManager.Instance.masterMuted;
                bool newSoundMuted = GUILayout.Toggle(
                    soundMuted, soundMuted ? "Unmute Sound" : "Mute Sound"
                );

                // If the state has changed, update the sound mute state
                if (newSoundMuted != soundMuted)
                {
                    SoundManager.Instance.masterMuted = newSoundMuted;
                }
                ...
                GUILayout.BeginHorizontal();
                {
                    ...
                    if (GUILayout.Button("Enable\nDummy\nCharacter",
                        ...
                        GUILayout.ExpandHeight(true)))
                    {
                        // Enable the dummy character using GameManager
